                        ...
                        <div class="col-xs-12">
                            <div class="bradcaump__inner">
                                <nav class="bradcaump-inner">
                                  ...
                                  <span class="breadcrumb-item active" style="color:orange">Whishlist</span>
                                </nav>
                            </div>
                        </div>
                    </div>
...
$res="select order_product.*,order_status.name as order_status_str from order_product,order_status where order_product.user_id='$uid' and order_status.id=order_product.order_status";
            $run_res=mysqli_query($con,$res);
        while ($row=mysqli_fetch_assoc($run_res)) {
                                            ?>
                                            <tr>
                                    <td class="product-add-to-cart">
                                        <a href="my_order_details.php?id=<?php echo $row['id']?>"><?php echo $row['id']?>:View Order</a>
                                        <br>
